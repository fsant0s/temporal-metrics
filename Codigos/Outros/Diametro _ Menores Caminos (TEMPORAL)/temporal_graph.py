...
# computes the diameter temporal
def diameterTemporal_averagePathShortest(G,start_time,end_time):
    ...
    for t in range(end_time,start_time-1,-1): # m time steps
        ...
        for v in verts: # n vertices
            ...
            for k_t in G.time_ordered_graph.neighbors((v,t)): # at most n
                ki = labels[k_t[0]]
                D_t[vi,ki] = 1
            if t < end_time:
    ...
    D_tplus1[D_tplus1 == inf] = 0
    diam = D_tplus1.max()
    D_tplus1 -= np.identity(n)
    D_tplus1 = (D_tplus1[D_tplus1 != 0])
    #print(D_tplus1)
    print("Quantidade de infinito = " + str(np.count_nonzero(D_tplus1 == inf)))
    print("Quant. menores caminhos = "+ str(np.count_nonzero(D_tplus1 >= 1)))
    return {"diameter": diam, "averShortestlen": np.mean(D_tplus1)}
