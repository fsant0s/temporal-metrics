...
def diameterTemporal(G,start_time,end_time):
    ...
    return {"diameter": D_tplus1.max(), "averShortestlen": np.mean(D_tplus1)}
...
def compute_temporal_degree(G,start_time,end_time):
    ...
    for v in verts:
        ...
        vi = labels[v]
        # edges - self_edges
        #print(G.time_ordered_graph.degree(v_t))
        degree[vi] = 0.5*sum( dict(G.time_ordered_graph.degree(v_t)).values()) - m
    degree = degree/((n-1)*m)
